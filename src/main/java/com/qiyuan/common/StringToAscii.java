...
//		System.out.println("str:"+str);
	...
	public static String crcTable(byte[] bytes) {
		...
		String str_m = Integer.toHexString(crc);
		String str = "0000";
	...
	public static byte[] getBytes(long cardno, String posId, int kneeId, int payType, int yuan, int jiaofen) {
		byte[] b = new byte[21];
		b[0] = 0x17;
		b[1] = (byte) 0xf0;
		byte[] cardBytes = getBytes(cardno, true);
		System.arraycopy(cardBytes, 0, b, 2, cardBytes.length);

		byte[] posByte = ByteUtil.strToBCD(posId, false);
		System.arraycopy(posByte, 0, b, cardBytes.length + 6 - posByte.length, posByte.length);

		b[cardBytes.length + 6] = intToByte(kneeId);
		b[cardBytes.length + 7] = intToByte(payType);
		byte[] yBytes = intToBytes(yuan);
		byte[] jBytes = intToBytes(jiaofen);
		b[cardBytes.length + 8] = yBytes[1];
		b[cardBytes.length + 9] = yBytes[2];
		b[cardBytes.length + 10] = jBytes[2];
		return b;
	}
	
	public static void main(String args[]) {
		String s1 = "22917787084";
		long l = Long.parseLong(s1);
		System.out.println("l:"+l);
		String s = Long.toHexString(l);
		System.out.println(s);
		
//		byte[] b = TypeConvert.getBytes(l, true);
//		for (int i = 0; i < b.length; i++) {
//			System.out.println(i+":"+b[i]);
//		}
		
		byte[] bytes = getBytes(l, "1", 0, 0, 250, 0);
		String crcString = crcTable(bytes);
		System.out.println("crc--->"+crcString);
		String asciiString = "";
		for (int i = 0; i < bytes.length; i++) {	
			asciiString = asciiString + parseAscii(toHex(bytes[i] & 0xff));
		}
		
		String ascii = asciiString + StringToAscii.parseAscii(crcString);
		System.out.println("ascii:"+ascii);
//		byte[] aa = { 0x13, 0x44, 0x00, 0x00, 0x00, 0x05, 0x56, 0x01,
//				(byte) 0xad, (byte) 0xcc, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
//				0x00 };
...
//		System.out.println("ת������ַ��ǣ�" + StringToAscii.parseAscii(toHex(0xcc)));
	}
