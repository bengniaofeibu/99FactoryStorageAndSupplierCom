...
public class ImageUtil {
	...
	public static String IMAGE_TYPE_JPG = "jpg";// 联合照片专家
	public static String IMAGE_TYPE_JPEG = "jpeg";// 联合照片专家
	public static String IMAGE_TYPE_BMP = "bmp";// 英文Bitmap（位图）的简写，它是Windows操作系统中的标准图像文件格式
	public static String IMAGE_TYPE_PNG = "png";// 可移植网络图
	public static String IMAGE_TYPE_PSD = "psd";// Photoshop的专用格式Photoshop

	public static boolean isImage(String path) {
		if (path.toLowerCase().lastIndexOf(".jpg") == (path.length() - 4))
			return true;
		if (path.toLowerCase().lastIndexOf(".png") == (path.length() - 4))
			return true;
		if (path.toLowerCase().lastIndexOf(".bmp") == (path.length() - 4))
			return true;
		return false;
	}
	...
	public final static void scale(String srcImageFile, String result, int scale, boolean flag) {
		try {
			...
			int width = src.getWidth(); // 得到源图片
			int height = src.getHeight(); // 得到源图片
			if (flag) {// 放大
				width = width * scale;
				height = height * scale;
			} else {// 缩小
	 ...
	 *            比例不对时是否需要补白：true为补白; false为不补白;
	 */
	public final static void scale3(String srcImageFile, String result, int height, int width) {
		try {
			double ratio1 = 0.0; // 缩放比例
			double ratio2 = 0.0; // 缩放比例
			File f = new File(srcImageFile);
			BufferedImage bi = ImageIO.read(f);
			Image itemp = bi.getScaledInstance(width, height, Image.SCALE_SMOOTH);
			// 计算比例
			ratio1 = (new Integer(width)).doubleValue() / bi.getWidth();
			ratio2 = (new Integer(height)).doubleValue() / bi.getHeight();
	...
	public final static void scale4(String srcImageFile, String result, int height, int width) {
		try {
			...
			Image itemp = bi.getScaledInstance(width, height, Image.SCALE_SMOOTH);
			// 计算比例
			double ratio = (new Integer(width)).doubleValue() / bi.getWidth();
			AffineTransformOp op = new AffineTransformOp(AffineTransform.getScaleInstance(ratio, ratio), null);
			itemp = op.filter(bi, null);
			ImageIO.write((BufferedImage) itemp, "JPEG", new File(result));
		} catch (IOException e) {
	...
	public final static void cut(String srcImageFile, String result, int x, int y, int width, int height) {
		try {
			...
			if (srcWidth > 0 && srcHeight > 0) {
				...
				BufferedImage tag = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
				Graphics g = tag.getGraphics();
				g.drawImage(img, 0, 0, width, height, null); // 绘制切割后的图片
				g.dispose();
				// 输出为文字
	 ...
	 *            源图像
	 * @param formatName
	 *            包含格式非正式名称的 String：如JPG、JPEG、GIF
	 * @param destImageFile
	 *            目标图像地址
	 */
	 ...
	 *            修正Y轴
	 * @param alpha
	 *            透明度：alpha 必须是范围 [0.0, 1.0] 之内（包含边界）的一个浮点数
	 */
